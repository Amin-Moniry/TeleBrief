...
MODEL_RETRIES = max(1, env_int("MODEL_RETRIES", 4))
FALLBACK_MODELS = [
    name.strip() for name in env_str("FALLBACK_MODELS").split(",") if name.strip()
]
...
def model_chain() -> list[str]:
    """Primary model first, then optional FALLBACK_MODELS, without duplicates."""
    return list(dict.fromkeys([XKIRO_MODEL, *FALLBACK_MODELS]))


def call_model(prompt: str, temperature: float = 0.15) -> Any:
    """Try each configured model; 429/5xx are transient and must back off."""
    last_error: Exception | None = None
    for model in model_chain():
        try:
            return call_single_model(model, prompt, temperature)
        except Exception as exc:
            last_error = exc
            logger.warning("مدل %s پاسخ معتبر نداد: %s", model, exc)
    raise RuntimeError(
        "سرویس مدل روی همه مدل‌های تنظیم‌شده در دسترس نیست"
    ) from last_error


def call_single_model(model: str, prompt: str, temperature: float) -> Any:
    last_error: Exception | None = None
    ...
    for attempt in range(MODEL_RETRIES):
        try:
            ...
            logger.warning(
                ...
                attempt + 1, MODEL_RETRIES, model, exc,
            )
            ...
            try:
                delay = float(retry_after) if retry_after else min(20, 3 * (2 ** attempt))
            except (TypeError, ValueError):
                delay = min(20, 3 * (2 ** attempt))
            time.sleep(delay + random.uniform(0.2, 1.2))
    raise RuntimeError(
        f"مدل {model} پس از {MODEL_RETRIES} تلاش پاسخ معتبر نداد"
    ) from last_error
...
async def analyze_messages(
    ...
    async def analyze_chunk(
        batch: list[ChannelMessage], depth: int = 0
    ) -> tuple[bool, list[dict[str, Any]]]:
        """A rejected chunk is retried in halves; a big payload often fails alone."""
        try:
            ...
            logger.error("یک دسته (%s پیام) تحلیل نشد: %s", len(batch), exc)
            if len(batch) < 2 or depth >= 3:
                return False, []
            middle = len(batch) // 2
            logger.info("دسته به دو نیمه کوچک‌تر تقسیم و دوباره تحلیل می‌شود")
            succeeded = False
            stories: list[dict[str, Any]] = []
            for half in (batch[:middle], batch[middle:]):
                half_ok, half_stories = await analyze_chunk(half, depth + 1)
                succeeded = succeeded or half_ok
                stories.extend(half_stories)
            return succeeded, stories

    async def analyze_batch(batch: list[ChannelMessage]) -> tuple[bool, list[dict[str, Any]]]:
        async with semaphore:
            return await analyze_chunk(batch)
